...
Unit coverage is in `lib/ornaments/geography.test.ts` and `lib/ornaments/geography-labels.test.ts`, including pan paths and changing pin order/visibility. Run `npm test`, `npx tsc --noEmit` and `npm run build`. The repository's existing lint failures in legacy effect-based state synchronization and the admin login anchor are outside this geographic-view change.
